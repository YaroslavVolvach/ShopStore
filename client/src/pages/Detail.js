...
function Detail() {
  ...
  useEffect(() => {
    ...
    // get cache from idb
      ...
      idbPromise('products', 'get').then(indexedProducts => dispatch({type: UPDATE_PRODUCTS, products: indexedProducts}));
    }
  ...
  const addToCart = () => {
    ...
    if (itemInCart) {
      dispatch({ type: UPDATE_CART_QUANTITY, _id: id, purchaseQuantity: parseInt(itemInCart.purchaseQuantity) + 1});
  ...
  const removeFromCart = () => { dispatch({type: REMOVE_FROM_CART, _id: currentProduct._id});
